...
test('using a FAT icon using array format', () => {
  const wrapper = render(
    <FontAwesomeIcon icon={['fa', 'arrow-alt-circle-down']} />
  )
  expect(wrapper.container.getElementsByTagName('svg').length).toEqual(1)
  const iconEle = wrapper.container.getElementsByTagName('svg')[0]
  expect(iconEle).toHaveClass('fa-arrow-alt-circle-down')
})
...
//   const vm =import { queryByDisplayValue, getByRole } from '../../../node_modules/@testing-library/dom/types/queries.d';
//  mount({import { getQueriesForElement } from '../../../node_modules/@testing-library/dom/types/get-queries-for-element.d';
